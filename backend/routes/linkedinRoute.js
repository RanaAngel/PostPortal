...
// const options = {
//   key: fs.readFileSync('path/to/52.20.87.194.key'),
//   cert: fs.readFileSync('path/to/52.20.87.194.crt')
// };
 ...
 const req_state = req.query.state;
  ...
  _request(method, hostname, path, headers, body).then(async (r) => {
    ...
    console.log(`LinkedIn token saved for user ID: ${user._id}`);
        ...
        res.redirect(`http://52.20.87.194:3000/dashboard?access_token=${access_token}&userId=${user._id}`);
