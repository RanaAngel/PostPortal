...
export default function CreatePost({ closeCreatePostModal }) {
    ...
    const [scheduleDate, setScheduleDate] = useState(null); // New state for scheduled date
    ...
    const handlePost = async () => {
        ...
        for (const platform of selectedPlatforms) {
            switch (platform) {
                case 'linkedin':
                    postTitle = 'linkedin'; 
                    await handleLinkedInPost(postTitle, scheduleDate ? scheduleDate : null);
                    break;
                case 'twitter':
                    postTitle = 'twitter';
                    await handlePostTweet(postTitle, scheduleDate ? scheduleDate : null);
                    break;
                case 'facebook':
                    postTitle = 'facebook'; 
                ...
                <div className="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                    <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        ...
                        />
                                    ...
                                    <button type="button" onClick={() => setScheduleDate(new Date())} className="inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:text-sm">
                                        Schedule Post
                                    </button>
